...
- Check `brain_sidecar_status` or `brain_diagnostic` when context quality matters — before non-trivial analysis, debugging, refactors, features, and docs audits
...
- After broad edits or generated files, confirm Brain can see new context with `brain_sidecar_status` or a targeted `brain_search`

## Workflow
1. **Health check** → `brain_sidecar_status` or `brain_diagnostic`
