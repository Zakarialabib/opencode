...
function checkElementBounds(slideData, sw, sh) {
  ...
  for (const el of slideData.elements) {
    ...
    if (p.x < -tol)
      w.push(`⚠ BOUNDS: "${getElementLabel(el)}" extends ${(-p.x * 72).toFixed(0)}pt beyond LEFT`);
    if (p.y < -tol)
      w.push(`⚠ BOUNDS: "${getElementLabel(el)}" extends ${(-p.y * 72).toFixed(0)}pt beyond TOP`);
    if (p.x + p.w > sw + tol)
      w.push(
...
function fixImageExtension(imagePath, tmpDir) {
  try {
    ...
    else if (buf[0] === 0x52 && buf[1] === 0x49 && buf[8] === 0x57 && buf[9] === 0x45)
      actualExt = ".webp";
