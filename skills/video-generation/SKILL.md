...
async function pollTaskUntilComplete(zai, taskId) {
  ...
  while (pollCount < maxPolls) {
    ...
    if (result.task_status === "SUCCESS") {
      return {
        success: true,
        url: result.video_result?.[0]?.url || result.video_url || result.url || result.video,
      };
    }
...
// Usage
const prompts = ["A cat playing with yarn", "A dog running in a park", "A bird flying in the sky"];
    ...
    if (result.task_status === "SUCCESS") {
      ws.send(
        JSON.stringify({
          type: "complete",
          taskId: taskId,
          videoUrl: result.video_result?.[0]?.url || result.video_url || result.url || result.video,
        })
      );
      break;
    }
