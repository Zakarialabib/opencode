...
async function preRenderHooks(page) {
  ...
  const overflows = await page.evaluate(() => {
    const out = [];
    document
      .querySelectorAll("pre,table,figure,img,svg,.mermaid,blockquote,.equation")
      .forEach((el) => {
  ...
  const vOverflowFix = await page.evaluate(() => {
    ...
    const finalHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight);
