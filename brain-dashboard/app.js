...
const tuningState = {
  ...
  rerankGate: 0.85,
  perIntent: {
    learn: { alpha: 0.35, beta: 0.50, gamma: 0.15 },
    debug: { alpha: 0.50, beta: 0.35, gamma: 0.15 },
    refactor: { alpha: 0.30, beta: 0.55, gamma: 0.15 },
    feature: { alpha: 0.25, beta: 0.60, gamma: 0.15 },
    test: { alpha: 0.40, beta: 0.45, gamma: 0.15 }
  }
};
...
const chatConfig = {
  ...
  systemPrompt: 'You are a helpful coding assistant. Use the provided context to answer questions about the codebase.'
};
...
app.get('/api/status', (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const ftsContent = db.exec("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'fts_chunks%'");
...
app.get('/api/memory/:concept', (req, res) => {
  ...
  const conceptData = memoryState.concepts.find(c => c.name.toLowerCase() === concept.toLowerCase());
...
app.post('/api/search', (req, res) => {
  ...
  try {
    ...
    const fusedResults = ftsResults.slice(0, Math.min(5, ftsResults.length)).map(r => ({
      ...
      score: parseFloat((alpha * Math.min(r.score / 100, 1) + beta * (0.85 + Math.random() * 0.1)).toFixed(2))
    }));
    ...
    const tokensPerChunk = Math.ceil(ftsResults.reduce((sum, r) => sum + (r.content?.length || 0), 0) / ftsResults.length);
...
app.get('/api/diagnostic', (req, res) => {
  ...
  pipelineData.totalDuration = pipelineData.stages.reduce(
...
app.get('/api/index', (req, res) => {
  ...
  try {
    ...
    if (indexState.progress.active) {
      indexState.progress.current = Math.min(indexState.progress.current + 1, indexState.progress.total);
...
app.post('/api/intent', (req, res) => {
  ...
  const learnPatterns = [
    ...
    { pattern: /\b(work|function|explain|understand|learn|know|show|what|describe)\b/i, score: 0.15 },
...
app.get('/api/tuning', (req, res) => {
  res.json({
    ...
    perIntent: {
      ...
      refactor: { alpha: tuningState.perIntent.refactor.alpha, beta: tuningState.perIntent.refactor.beta },
      feature: { alpha: tuningState.perIntent.feature.alpha, beta: tuningState.perIntent.feature.beta },
...
app.post('/api/tuning', (req, res) => {
  ...
  if (updates.intent) {
    if (!['learn', 'debug', 'refactor', 'feature', 'test'].includes(updates.intent)) {
      return res.status(400).json({ error: 'Invalid intent. Must be one of: learn, debug, refactor, feature, test' });
    }
...
app.get('/api/tracer', (req, res) => {
  const totalDecisions = Object.values(tracerState.intentCounts).reduce((sum, count) => sum + count, 0);
...
app.post('/api/tracer', (req, res) => {
  ...
  const newDecision = {
    id: tracerState.decisions.length > 0 ? Math.max(...tracerState.decisions.map(d => d.id)) + 1 : 1,
    time: time,
    intent: intent,
    query: query,
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    if (!lmStudioAvailable) {
      const intentMatch = message.match(/\b(how\s+(does|do|can)|what|why|explain|fix|add|implement)/i);
...
app.post('/api/budget', (req, res) => {
  ...
  if (maxTokens !== undefined) {
    ...
    if (typeof targetEfficiency !== 'number' || targetEfficiency < 0.1 || targetEfficiency > 0.99) {
      return res.status(400).json({ error: 'targetEfficiency must be a number between 0.1 and 0.99' });
    }
    delta = 0;
